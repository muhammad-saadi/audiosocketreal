class Api::V1::Collaborator::TracksController < Api::V1::Collaborator::BaseController
  ...
  def index
    ...
    render json: @tracks.includes(%i[publisher mp3_file_attachment wav_file_attachment aiff_file_attachment], artists_collaborator: :collaborator),
  ...
  def track_params
    params.permit(:title, :mp3_file, :wav_file, :aiff_file, :public_domain, :status, :lyrics, :explicit, :parent_track_id, track_publishers: %i[publisher_id percentage], track_writers: %i[artists_collaborator_id percentage])
  end
