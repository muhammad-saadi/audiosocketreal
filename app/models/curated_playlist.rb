class CuratedPlaylist < ApplicationRecord
  ...
  def set_order
    below_order = CuratedPlaylist.where("curated_playlists.order > ?", self.order)
    CuratedPlaylist.decrement_counter(:order, below_order)
  end

  def order_limit
    errors.add(:order, 'Order out of bounds') if order > CuratedPlaylist.maximum('order').to_i
    if order_was > order
      below_order = CuratedPlaylist.where("curated_playlists.order >= ? and curated_playlists.order <= ?", order, order_was).without(self)
      CuratedPlaylist.increment_counter(:order, below_order)
    elsif order_was < order
      above_order = CuratedPlaylist.where("curated_playlists.order <= ? and curated_playlists.order >= ? ", order, order_was).without(self)
